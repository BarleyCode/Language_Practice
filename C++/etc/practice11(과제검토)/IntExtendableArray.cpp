...
bool IntExtendableArray::extend(int capacity) {
     ...
     * 확장이 완료되면 true 반환.
     */
   int newSize = this->size() + capacity;
...
bool IntExtendableArray::add(int n) {
   ...
   int newCapacity = (this->size() + INIT_SIZE) > MAX_SIZE ? (MAX_SIZE - this->size()): INIT_SIZE;

   if (extend(newCapacity)) {
      set(nextIndex, n); // 인덱스가 전체 사이즈의 다음 -> 인덱스가 전체 사이즈 값이 됨
      return true;
   } else return false;
}