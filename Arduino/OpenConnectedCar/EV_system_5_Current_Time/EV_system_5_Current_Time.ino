...
void loop() {
  ...
  uint16_t blinkcounter = 0;              // Adafruit LCD 가운데 2개 점 깜박임 카운트
  uint16_t current_time = 1000;            // 현재 시간 표시
  int current_time_second = 0;            // 현재 시간 second 계산
  ...
  else
  {
    ...
    while(idle_state_exit != 1)
    {
      ...
      if(current_time_second >= 60)   // 1분 지났으면
      {
        current_time++;
        if((current_time % 100) / 10 == 6)
        {
          if(current_time == 1260)
          {
            current_time = 100;
          }
          else
          ...
          matrix.writeDigitNum(0, (current_time / 1000), drawDots);
          matrix.writeDigitNum(1, (current_time / 100) % 10, drawDots);
          matrix.drawColon(drawDots);
          matrix.writeDigitNum(3, (current_time % 100) / 10 , drawDots);
          matrix.writeDigitNum(4, current_time % 10, drawDots);
          matrix.writeDisplay();
          delay(997);
        }
        else
        {
          ...
          matrix.writeDigitNum(1, (current_time / 100) % 10, drawDots);
          matrix.drawColon(drawDots);
          matrix.writeDigitNum(3, (current_time / 10) % 10, drawDots);
          matrix.writeDigitNum(4, current_time % 10, drawDots); 
          matrix.writeDisplay();
          blinkcounter+=50;         // 깜박임 카운트 증가
          current_time_second++;    // 초 증가
          if (blinkcounter < 500) {
            drawDots = false;
